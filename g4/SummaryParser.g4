...
parser grammar SummaryParser;

options {
    // 表示解析token的词法解析器使用SummaryLexer
    tokenVocab = SummaryLexer;
}

// EOF(end of file)表示文件结束符，这个是Antlr中已经定义好的
//prog: expressions | STAR EOF;
//line: expressions | other EOF;
...
point: leftExpr = JUDIAN operator = EQ rightExpr = ALL_CONTENT;
module: leftExpr = ZUJIAN operator = EQ rightExpr = ALL_CONTENT;
type: leftExpr = FENLEI operator = EQ rightExpr = ALL_CONTENT;
status: leftExpr = ZHUANGTAI operator = EQ rightExpr = ALL_CONTENT;
issue: leftExpr = WENTI operator = EQ rightExpr = ALL_CONTENT;
jira: leftExpr = DANHAO operator = EQ rightExpr = ALL_CONTENT;
...
expression:
    ...
    | other
    ;
// 局点=西安  或者  局点:西安 或者  局点：西安
//    leftExpr = JUDIAN operator = EQ rightExpr = ALL_CONTENT       #point
//    | leftExpr = ZUJIAN operator = EQ rightExpr = ALL_CONTENT     #module
//    | leftExpr = FENLEI operator = EQ rightExpr = ALL_CONTENT     #type
//    | leftExpr = ZHUANGTAI operator = EQ rightExpr = ALL_CONTENT  #status
//    | leftExpr = WENTI operator = EQ rightExpr = ALL_CONTENT      #issue
//    | leftExpr = DANHAO operator = EQ rightExpr = ALL_CONTENT     #jira
//    ;

expressions:
    // 局点=西安;问题=系统故障;  或者  局点:西安。问题:系统故障。
    (expression (SEMI | FULLSTOP))+
    ;
